...
ADodgeballCharacter::ADodgeballCharacter()
{
	...
	// Игнорирование вращения камеры в зависимости от ориентации персонажа
...
void ADodgeballCharacter::LoseHeals(float Amount)
{
	Health -= Amount;
	ADodgeballPlayerController* PlayerController = Cast<ADodgeballPlayerController>(GetController());
	PlayerController->UpdateHealthPercent(GetHealthPercent());
	if (Health <= 0.0f)
	{
		Health = 0.0f;
		if(PlayerController != nullptr)
		{
			...
			PlayerController->RemoveHealthBar();
		}
	}
}
